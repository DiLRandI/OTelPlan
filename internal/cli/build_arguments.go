...
func parseBuildArguments(args []string) (buildArguments, error) {
	...
	for i := 0; i < len(args); i++ {
		...
		case "-o", "-p", "-tags", "-mod", "-modfile":
			...
			case "-mod":
				if value != "mod" && value != "readonly" && value != "vendor" {
					return buildArguments{}, errors.New("invalid build module mode")
				}

				result.AnalysisFlags = append(result.AnalysisFlags, name+"="+value)
			case "-modfile":
				if !strings.HasSuffix(value, ".mod") {
					return buildArguments{}, errors.New("build module file requires a .mod extension")
				}
		...
		default:
			return buildArguments{}, errors.New("unsupported build flag; source-selection and compiler overrides require analysis support")
		}
	}
