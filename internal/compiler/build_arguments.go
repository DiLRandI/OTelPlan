	...
	for i := 0; i < len(args); i++ {
		...
		case "-o", "-p", "-tags", "-mod":
			...
			if name == "-tags" && !slices.Equal(normalizedBuildTags(strings.FieldsFunc(value, func(r rune) bool { return r == ',' || unicode.IsSpace(r) })), normalizedBuildTags(build.BuildTags)) {
				return errors.New("build tags differ from analysis")
			}

			if name == "-mod" && value != build.ModuleMode {
				return errors.New("build module mode differs from analysis")
			}
		case "-a", "-v", "-x":
			if hasValue && value != "true" && value != "false" {
				return errors.New("invalid boolean build flag")
			}
		default:
			return errors.New("unsupported build flag; compiler and source-selection overrides require analysis support")
		}
	}
