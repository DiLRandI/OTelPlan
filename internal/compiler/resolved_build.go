...
func BuildResolved(ctx context.Context, request ResolvedBuildRequest) (BuildResult, error) {
	if request.DefaultOutput && request.DirectoryOutput {
		return BuildResult{}, fmt.Errorf("build output modes are mutually exclusive")
	}
	if request.Code == nil {
		return BuildResult{}, fmt.Errorf("build requires analysis")
	}
	if request.Code.EffectiveBuild.ModuleMode == "vendor" {
		return BuildResult{}, fmt.Errorf("vendored builds require isolated vendor materialization")
	}
	...
	for _, arg := range request.GoArgs {
		...
		if name == "-o" || name == "--o" {
			return BuildResult{}, fmt.Errorf("resolved build owns its temporary output path")
		}
	}
	if request.Parent != "" {
	...
	if err != nil && request.Code.EffectiveBuild.Workspace && filepath.Clean(workingDir) == filepath.Dir(request.Code.WorkspaceFile) {
		if buildPackageStart(request.GoArgs) >= len(request.GoArgs) {
			return BuildResult{}, fmt.Errorf("workspace-root build requires explicit package targets")
		}
