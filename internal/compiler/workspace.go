...
func PrepareWorkspace(ctx context.Context, request WorkspaceRequest) (PreparedWorkspace, error) {
	...
	for i, dir := range dirs {
		if !filepath.IsAbs(dir) || filepath.Clean(dir) != dir || (i > 0 && dirs[i-1] == dir) {
			return PreparedWorkspace{}, errors.New("source module directories must be unique absolute paths")
		}
	}

	if len(dirs) == 0 {
		return PreparedWorkspace{}, errors.New("build workspace requires application modules")
	}

	for dir, path := range request.AlternateModFiles {
		if !slices.Contains(dirs, dir) || !filepath.IsAbs(path) || !strings.HasSuffix(path, ".mod") {
			return PreparedWorkspace{}, errors.New("alternate module file requires a known module and an absolute .mod path")
		}
	}
	...
	for _, dir := range dirs {
		...
		if alternate, ok := request.AlternateModFiles[dir]; ok {
			err := installAlternateModuleFiles(alternate, result.Relocations[dir])
			if err != nil {
				return PreparedWorkspace{}, err
			}
		}
	...
	if len(request.WorkspaceSums) > 0 {
		err := os.WriteFile(result.WorkspaceFile+".sum", request.WorkspaceSums, 0o600)
		if err != nil {
