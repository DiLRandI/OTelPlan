...
func RelocateWorkspaceManifest(data []byte, originalDir, runtimeDir string, relocations map[string]string) ([]byte, error) {
	if !filepath.IsAbs(originalDir) || !filepath.IsAbs(runtimeDir) {
		return nil, errors.New("workspace directories must be absolute")
	}
	...
	relocated := func(path string) (string, error) {
		...
		if !ok || !filepath.IsAbs(result) {
			return "", errors.New("workspace path has no isolated copy")
		}
	...
	for _, use := range work.Use {
		...
		if seen[path] {
			return nil, errors.New("workspace has duplicate copied modules")
		}
	...
	if seen[runtimeDir] {
		return nil, errors.New("runtime module overlaps application workspace")
	}
