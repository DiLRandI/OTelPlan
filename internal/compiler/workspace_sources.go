...
func CollectWorkspaceSources(ctx context.Context, workspace []byte, workspaceDir string, alternateModFiles map[string]string) ([]string, error) {
	...
	for dir, path := range alternateModFiles {
		if !filepath.IsAbs(dir) || filepath.Clean(dir) != dir || !filepath.IsAbs(path) || !strings.HasSuffix(path, ".mod") {
			return nil, errors.New("alternate module file requires absolute module and .mod paths")
		}
	}
	...
	if len(work.Use) == 0 {
		return nil, errors.New("workspace requires application modules")
	}
	...
	for len(pending) > 0 {
		...
		if err != nil {
			return nil, errors.New("resolve source module directory")
		}

		if previous, ok := physicalDirs[physical]; ok && previous != dir {
			return nil, errors.New("source module is referenced through different filesystem aliases")
		}
		...
		if err != nil {
			return nil, errors.New("read source module manifest")
		}
		...
		if module.Module == nil {
			return nil, errors.New("source module manifest requires module directive")
		}
	...
	for dir := range alternateModFiles {
		if !seen[dir] {
			return nil, errors.New("alternate manifest references an unknown source module")
		}
	}
